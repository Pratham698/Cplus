...
// Defining all methods //
  ...
  private void update_Leave(){
      ...
      String leaveId = option.next();
        ...
        System.out.println(Leave.listById(leaveid).getEmpId());
    System.out.println();
    ...
    String r = null;
    try {
      r = Leave.update_leave(empid, sDate,
  ...
  private void approve_denyLeave(){
    ...
    String ch = sc.next();
    String res = Leave.approveDeny(leaveId, mgrId, ch);
    System.out.println(res);
    sc.close();
  }
  ...
  private void pending_leavestatus(){
    ...
    Leave[] pending = Leave.listPending(mgrId);
    for (Leave e : pending) {
      System.out.println(e.getLeaveId() + " | " + e.getEmployeeId() + " | "
          + e.getLeaveFrom() + " | " + e.getLeaveTo() + " | " + e.getNoLeaveDays() + " | "
          + e.getLeaveType() + " | " + e.getLeaveStatus() + "| " + e.getLeaveReason()
  ...
  private void listManagerDetails() {
   ...
   try{
    ...
    if (e != null){
      final Employee[] employee = Employee.listManagerById(empId);
